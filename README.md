...
统一通知管线驱动邮件、浏览器推送与 QQ 机器人三渠道：被指派/改派待办、待办完成、现场任务分配、重要/紧急公告即时通知，异常上报与新风险直达管理者；待办节点/到期、里程碑临近、每周周报由 cron 统一提醒。配置 VAPID 后推送直达设备通知栏，点击跳转对应页面；未配置时静默回退纯邮件。配置 QQ 机器人（QQ 开放平台 Bot API v2）后，成员在「我的」页生成绑定码、QQ 私聊机器人发送「绑定 XXXXXX」即可在 QQ 单聊收取全部个人通知；项目「设置」页可生成群绑定码，@机器人 绑定后群里收精选类型（里程碑临近/待办提醒/公告/风险/异常/周报），不刷高频消息；群里或单聊里直接给机器人发任务描述还能由 AI 解析自动创建待办（相对时间自动换算、群里 @成员 自动指派，单聊属于多个进行中活动时回复序号选择归属）。

...
export default function Login() {
  ...
  return (
    <AuthShell
      title="登录"
      description="登录你的账号"
      footer={<>没有账号？<Link to="/register" className="text-primary hover:underline">使用邀请码注册</Link></>}
    >
      {isDemo && (
        <div className="mb-4 space-y-3 rounded-lg border border-warning/40 bg-warning-soft p-3">
          <p className="text-sm text-foreground">这是功能预览演示，无需账号，点击下方按钮直接进入</p>
          <Button
            type="button"
            variant="outline"
            className="w-full border-warning/60"
            disabled={busy}

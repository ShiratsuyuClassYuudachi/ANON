...
显示项目名称、状态徽章（筹备中 / 进行中 / 已结束）、倒计时（距开始或距结束的天/小时数）、阶段与地点。倒计时颜色随紧迫度变化：正常 → 警告（≤7 天）→ 进行中 → 已结束。
...
- **视图切换**：顶部「个人视图」/「项目视图」切换按钮。个人视图侧重待我处理与个人日程；项目视图侧重全局风险与模块统计。默认值根据角色自动设定（管理者默认项目视图，普通成员默认个人视图）
